...
class MemberService {
    ...
    public async login(input: LoginInput): Promise<Member> {
        ...
        .findOne( 
            {  memberNick: input.memberNick,
               memberStatus: { $ne: MemberStatus.DELETE}, //$ne: degani shundan boshqasini top degani
            },  // birinchisi FILTER 
            { memberNick: 1, memberPassword: 1, memberStatus: 1 }  // 2chisi Projection buladi
        )
        .exec();
    ...
    public async addMemberPoint(member: Member, point: number): Promise<Member> {
        const memberId = shapeIntoMongooseObjectId(member._id);
  ...
  /** SSR */
    ...
    public async processLogin(input: LoginInput): Promise<Member> {
        const member = await this.memberModel
        .findOne( 
            {memberNick: input.memberNick},  // birinchisi FILTER 
            { memberNick: 1, memberPassword: 1 }  // 2chisi Projection buladi
        )
        .exec();
        if (!member) throw new Errors(HttpCode.NOT_FOUND, Message.NO_MEMBER_NICK);
